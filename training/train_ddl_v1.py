...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_TRANSACTIONS.TXN_STATUS This field describes the outcome of a US Stock buy or sell order. examples {"ORDER_CANCELED": Order canceled by user., "ORDER_SUCCESS": Order fulfilled by broker., "ORDER_INITIATION_FAILED": Order failed to be placed with vendor (likely technical issue)., "ORDER_FAILED": Order placed but not fulfilled.}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_USER_BASE_FACT.ACCOUNT_STATUS This column indicates whether a user\'s US Stocks account has been successfully created. Account creation is complete only when the status is "ACTIVE". examples {"MANUAL_INTERVENTION": Account creation stuck due to a technical issue., "INITIATED": Account creation started but not active., "KYC_VERIFIED": Account not active yet., "CREATION_ON_HOLD": Account not active yet., "ACTIVE": Account is active and ready to use., "ACCOUNT_STATUS_UNSPECIFIED": Account not active yet., "CREATED": Account created but not yet active.}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_USER_BASE_FACT.SUITABILITY_FORM_RISK_LEVEL_OPTED This is the risk level the user opted for in the US Stocks suitability form. It indicates their willingness to take risks in investments. examples {"low-risk": Prefers investments with low risk, "medium-risk": prefers investments with medium risk, "high-risk": prefers investments high risk}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_USER_BASE_FACT.SUITABILITY_FORM_EMPLOYMENT_TYPE This column records the user\'s employment type, as selected during the US Stocks suitability assessment. examples {"homemaker-employment": Homemaker, "business-owner-professional-employment": Business Owner/Professional, "others-employment": Other, "salaried-with-less-than-6-months-experience-employment": Salaried (<6 months experience), "working-professional-employment": Working Professional, "retired-employment": Retired, "student-employment": Student, "self-employed-or-freelancer-employment": Self-Employed/Freelancer}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_USER_BASE_FACT.ACCT_CREATION_WORKFLOW_STAGE This column shows the user\'s current step in the US Stocks account creation process. It provides finer detail than the overall ACCOUNT_STATUS. examples {"US_BROKER_ACCOUNT_OPENING_COLLECT_AGREEMENTS": Consenting to agreements, "US_BROKER_ACCOUNT_OPENING_MANUAL_PAN_REVIEW": PAN under review, "US_BROKER_ACCOUNT_OPENING_COLLECT_SOURCE_OF_FUNDS": Submitting source of funds, "US_BROKER_ACCOUNT_OPENING_MANUAL_PAN_UPLOAD": Uploading PAN, "US_BROKER_ACCOUNT_OPENING_COLLECT_POA_AND_POI": Consenting to PoA/PoI collection, "USS_ACCOUNT_CREATION_ON_HOLD": Account on hold (requirements not met), "US_BROKER_ACCOUNT_OPENING_UPLOAD_KYC_DATA": Uploading KYC data, "US_BROKER_ACCOUNT_OPENING_COLLECT_RISK_DISCLOSURE": Providing risk disclosure, "US_BROKER_ACCOUNT_OPENING_COLLECT_EMPLOYMENT_DETAILS": Submitting employment details, "US_BROKER_ACCOUNT_OPENING_ACCOUNT_OPENING_WITH_VENDOR": Account opening in progress with broker, "US_BROKER_ACCOUNT_OPENING_AML_CHECK": AML check in progress, "US_BROKER_ACCOUNT_OPENING_WAIT_FOR_DOCUMENTS_UPLOAD": Waiting for documents, "US_BROKER_ACCOUNT_OPENING_POLL_ACCOUNT_STATUS": Checking account status with vendor, "US_BROKER_ACCOUNT_OPENING_FETCH_ADDRESS": Retrieving user\'s address}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_USER_BASE_FACT.DISTINCT_STOCKS_WATCHLISTED_CURRENT This represents the list of stocks that are watchlisted by the user at present. examples { "AAPL, AXP, CVX, KHC, KO, MCO, OXY": User has AAPL, AXP, CVX, KHC, KO, MCO, OXY in watchlist}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_USER_BASE_FACT.DISTINCT_STOCKS_VISITED This represents the list of stocks that are visited by the user ever. examples {"AAPL, AXP, CVX, KHC, KO, MCO, OXY": User has visited AAPL, AXP, CVX, KHC, KO, MCO, OXY}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATALAKE_TECH.VENDORMAPPING.DP_VENDOR_MAPPINGS.MOENGAGE_ID It represents a user for the communication tool Moengage. Typically, this is used for sending any communications related to the user. examples {"0f8f515b-9cee-4f71-af1c-e213dcc33e15"}'
)
...
vn.train(
    ddl='EPIFI_DATALAKE_ALPACA.USSTOCKS_ALPACA.STOCKS.ID It is the internal stock ID which is the primary key of this table. This same column may be defined in other tables as Stock_ID. examples {"USS221228kEOfGr9/Q1+eDxOEDTfj3A=="}'
)
vn.train(
    ddl='EPIFI_DATALAKE_ALPACA.USSTOCKS_ALPACA.STOCKS.SYMBOL This is the stock symbol with which it trades on the exchange. This can also be used as a primary key and joining key across tables. examples {"NXGN": Symbol for NextGen Healthcare}'
)
vn.train(
    ddl='EPIFI_DATALAKE_ALPACA.USSTOCKS_ALPACA.STOCKS.EXCHANGE Exchange represents the exchange where the symbol is listed and where the orders for it will be routed. examples {"EXCHANGE_NYSE": New York Stock Exchange, "EXCHANGE_NAS": NASDAQ, "EXCHANGE_ASE": American Stock Exchange, "EXCHANGE_ARCA": NYSE Arca, mostly ETFs are listed here, "EXCHANGE_BATS": mostly ETFs are listed here}'
)
...
vn.train(
    ddl='EPIFI_DATALAKE_ALPACA.USSTOCKS_ALPACA.STOCKS.INTERNAL_STATUS It defines whether the stock is available to buy or not. examples {"INTERNAL_STATUS_UNAVAILABLE": Unavailable to buy/ sell, "INTERNAL_STATUS_AVAILABLE": Available to buy/sell, "INTERNAL_STATUS_UNSPECIFIED": where availability is unknown, "INTERNAL_STATUS_DISABLED_BUY": Sell is allowed but buy is disabled}'
)
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_USER_DAILY.ACTOR_ID This is the identifier for a user across the organization. It can be used as a joining key to connect with user data in other tables. examples {"35479187-1a5a-4dd2-80fd-695c2eb3f657"}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_USER_DAILY.FAILURE_REASON_ADD_FUND This column shows the reason why an "add funds" transaction failed, if any. It\'s null if there were no failed attempts on the given date. examples {"WALLET_ORDER_FAILURE_REASON_OUTWARD_SWIFT_TRANSFER_FAILED": SWIFT transfer failed., "WALLET_ORDER_FAILURE_REASON_LRS_LIMIT_BREACHED": RBI\'s $250,000 remittance limit reached., "WALLET_ORDER_FAILURE_REASON_BREACHED_MAX_ALLOWED_LIMIT_TO_ADDED_FUNDS_FOR_DAY": Daily remittance limit reached., "WALLET_ORDER_FAILURE_REASON_PAN_CHECK_FAILED_WITH_BANKING_PARTNER": PAN ineligible for remittance (KYC needed)., "WALLET_ORDER_FAILURE_REASON_INSUFFICIENT_NO_OF_TRANSACTIONS": Bank account too inactive., "WALLET_ORDER_FAILURE_REASON_KYC_CHECK_FAILED_WITH_BANKING_PARTNER": KYC check failed., "WALLET_ORDER_FAILURE_REASON_FOREIGN_REMITTANCE_NOT_ALLOWED": Remittance not allowed by bank., "WALLET_ORDER_FAILURE_REASON_BREACHED_MAX_ALLOWED_LIMIT_TO_ADDED_FUNDS_IN_FINANCIAL_YEAR": Regulatory limit (suitability) breached., "WALLET_ORDER_FAILURE_REASON_SOF_REMITTANCE_LIMIT_BREACHED": Source of Funds limit breached., "null": No add funds failures.}'
)
vn.train(
...
vn.train(
    ddl="EPIFI_DATAMART_ALPACA.USSTOCKS.USS_WALLET_ORDERS.ACTOR_ID This is the identifier for a user across the organization. It represents the user who initiated the wallet transaction and can be used as a joining key to connect with user data in other tables."
)
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_WALLET_ORDERS.USS_ACCOUNT_ID This is the unique identifier for a user\'s US Stocks brokerage account that is opened with the vendor. It represents the account associated with the wallet transaction. examples {"USSACCpp3AQIkLSTutA0M6dEsT8A240702=="}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_WALLET_ORDERS.STATUS This field shows the current status of the wallet order (add funds or withdraw funds). examples {"WALLET_ORDER_STATUS_CREATED": Order created, processing pending., "WALLET_ORDER_STATUS_FAILED": Order failed., "WALLET_ORDER_STATUS_SUCCESS": Order successfully completed., "WALLET_ORDER_STATUS_MANUAL_INTERVENTION": Order requires manual intervention by an engineer., "WALLET_ORDER_STATUS_INITIATION_FAILED": Order initiation failed.}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_WALLET_ORDERS.WALLET_ORDER_TYPE_DERIVED This is a cleaner, more readable version of the \'WALLET_ORDER_TYPE\' column. It provides a user-friendly representation of the wallet order type. examples {"Add Funds", "Withdraw Funds"}'
)
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_WALLET_ORDERS.WALLET_ORDER_SUB_TYPE This field provides a more granular classification of the wallet order type. examples {"WALLET_ORDER_SUB_TYPE_UNSPECIFIED": Used for older wallet orders; can generally be ignored., "WALLET_ORDER_SUB_TYPE_ADD_FUNDS_REWARDS": Represents a reward credit, "WALLET_ORDER_SUB_TYPE_NON_INSTANT_WALLET_WITHDRAWAL": Represents a standard withdrawal transaction from the wallet., "WALLET_ORDER_SUB_TYPE_ADD_FUNDS_NON_INSTANT_WALLET_FUNDING": Represents an add funds transaction to the user\'s wallet.}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_WALLET_ORDERS.LATEST_WF_STAGE This field shows the latest stage the wallet transaction\'s workflow has reached. Different workflows are used for Add Funds, Withdraw Funds, and Rewards. examples {"TRACK_ORDER": (Reward) Checking if reward credited., "SEND_BUY_ORDER": (Reward) Buy order sent after reward credit., "TRACK_WALLET_FUND_TRANSFER": (Add Funds) Tracking transfer and crediting to wallet., "REFUND_PAYMENT": (Add Funds) Refund in progress (failed transfer)., "POOL_ACCOUNT_TRANSFER": (Add Funds) Payment credited to bank\'s pool account., "FOREIGN_FUND_TRANSFER": (Add Funds) International transfer in progress., "TRACK_INWARDS_REMITTANCE_STATUS": (Withdraw Funds) Tracking incoming remittance.}'
)
vn.train(
...
vn.train(
    ddl='EPIFI_DATAMART_ALPACA.USSTOCKS.USS_WALLET_ORDERS.FAILURE_REASON This field provides the reason for a failed wallet order, if applicable. It will be null for successful orders. examples {"WALLET_ORDER_FAILURE_REASON_KYC_CHECK_FAILED_WITH_BANKING_PARTNER": KYC check failed at bank\'s end for the customer., "WALLET_ORDER_FAILURE_REASON_SOF_REMITTANCE_LIMIT_BREACHED": Source of Funds limit exceeded., "WALLET_ORDER_FAILURE_REASON_BREACHED_MAX_ALLOWED_LIMIT_TO_ADDED_FUNDS_IN_FINANCIAL_YEAR": Yearly add funds limit reached set by the organisation., "WALLET_ORDER_FAILURE_REASON_LRS_LIMIT_BREACHED": RBI LRS limit exceeded., "WALLET_ORDER_FAILURE_REASON_FOREIGN_REMITTANCE_NOT_ALLOWED": Foreign remittance not allowed., "WALLET_ORDER_FAILURE_REASON_PAN_CHECK_FAILED_WITH_BANKING_PARTNER": PAN check failed., "WALLET_ORDER_FAILURE_REASON_INSUFFICIENT_NO_OF_TRANSACTIONS": Insufficient bank account activity., "WALLET_ORDER_FAILURE_REASON_BREACHED_MAX_ALLOWED_LIMIT_TO_ADDED_FUNDS_FOR_DAY": Daily add funds limit reached., "WALLET_ORDER_FAILURE_REASON_ERROR_TRANSFERRING_AMOUNT_TO_POOL_ACCOUNT": Error transferring to pool account because the payment has failed., "WALLET_ORDER_FAILURE_REASON_OUTWARD_SWIFT_TRANSFER_FAILED": Outward SWIFT transfer failed., "WALLET_ORDER_FAILURE_REASON_UNSPECIFIED": Unspecified withdraw funds failure reason., null: No failure reason. Order successful.} '
)
vn.train(
